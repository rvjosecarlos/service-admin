...
export default function ServicioForm({ register, errors, children  }: ServicioFormProps){
    ...
    const handleSelectDrivers = async () => {
        ...
        const result = ConductoresArrSchema.safeParse(conductores.data);
        if( result.success ){
                ...
                <div className="p-3 space-y-5">
                    ...
                    <div>
                        ...
                        <input 
                            id="descripcion" 
                            type="text" 
                            className={`p-1 w-full border border-borderColor placeholder:text-inputColor rounded focus:outline-none focus:ring-2 focus:border-ringColor ${errors.description && "border-2 border-destructiveColor"}`}
